...
describe("Combined machines on a shared board", () => {
  test("each machine computes score correctly in a single board layout", () => {
    ...
    expect((board[0] as Jet).score(board)).toBe(8);
    expect((board[1] as Train).score(board)).toBe(1);
    expect((board[4] as Ambulance).score(board)).toBe(12);
    expect((board[6] as SlotMachine).score(board)).toBe(0);
    expect((board[7] as Helicopter).score(board)).toBe(4);
    expect((board[8] as Missile).score(board)).toBe(2);
  });
});
