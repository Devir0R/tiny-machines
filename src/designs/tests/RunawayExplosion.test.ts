import { RunawayExplosion } from "../pairs/RunawayExplosion";
import { Train } from "../../machines/Train";
import { Missile } from "../../machines/Missile";

const createEmptyBoard = () => Array(9).fill(null) as (any | null)[];

describe("RunawayExplosion", () => {
  test("scores 15 points for each Train adjacent to a Missile", () => {
    const board = createEmptyBoard();
    const design = new RunawayExplosion();
    board[4] = new Train(4);
    board[5] = new Missile(5); // adjacent to 4

    expect(design.score(board)).toBe(15);
  });

  test("scores 0 when Train and Missile are not adjacent", () => {
    const board = createEmptyBoard();
    const design = new RunawayExplosion();
    board[0] = new Train(0);
    board[8] = new Missile(8); // not adjacent

    expect(design.score(board)).toBe(0);
  });

  test("scores 30 for two pairs", () => {
    const board = createEmptyBoard();
    const design = new RunawayExplosion();
    board[0] = new Train(0);
    board[1] = new Missile(1); // adjacent
    board[3] = new Train(3);
    board[4] = new Missile(4); // adjacent

    expect(design.score(board)).toBe(30);
  });

  test("scores 0 when no machines are present", () => {
    const board = createEmptyBoard();
    const design = new RunawayExplosion();

    expect(design.score(board)).toBe(0);
  });
});